...
| $$ Ax = \lambda{x} $$|
|-|
|- 일반적으로 x가 A를 만나게 되면(Ax) 원래의 x와는 다른 방향으로 변환된다. <br>- 간혹, x가 A를 만나도 이전과 동일or평행(parallel)한 벡터들이 존재한다. <br> - 이것이 **고유벡터(eigenvector)**이다: $$A_X$$ parallel to $$_X$$  <br> -  여기서 크기를 나타내는 특정 상수는 람다(λ, lambda)로 표현되었으며, 람다가 바로 고유값(eigenvalue)이다. |
